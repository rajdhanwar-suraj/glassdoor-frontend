    ...
    const fetchCompanies = async () => {
        try {
            const queryParams = new URLSearchParams({
                // Add query parameters based on user selections or filters
                // Example: name, companyType, location, rating
            });

            const response = await fetch(`https://glassdoor-backend.vercel.app/api/company/getAllCompany?${queryParams.toString()}`);
            if (!response.ok) {
                throw new Error('Failed to fetch companies');
            }
            const data = await response.json();
            setCompanies(data.companies);  // Assuming your API response has 'companies' array
        } catch (error) {
